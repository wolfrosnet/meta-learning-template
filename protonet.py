...
if __name__ == '__main__':
    # --shot 5 --train-way 20 for 5Shot 5Ways
    # --shot 1 --train-way 30 for 1Shot 5Ways
    ...
    wandb.init(
        entity="auroraveil",
        project="F33lLik3Dyin",
        name=f'OGprotonet_{run_id}',
        config={
            "max-epoch":args.max_epoch,
            "shot":args.shot,
            "test-way":args.test_way,
            "test-shot":args.test_shot,
            "test-query":args.test_query,
            "train-query":args.train_query,
            "train-way":args.train_way,
            "seed":args.seed,
        },   
    )
    ...
    path_data = '/data/mghong/workspace/reprod/dataset'
    train_dataset = l2l.vision.datasets.MiniImagenet(
        root=path_data, mode='train', download=False)
    valid_dataset = l2l.vision.datasets.MiniImagenet(
        root=path_data, mode='validation', download=False)
    test_dataset = l2l.vision.datasets.MiniImagenet(
        root=path_data, mode='test', download=False)
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        train_loss, train_acc = n_loss/loss_ctr, n_acc/loss_ctr
        ...
        print('epoch {}, val, loss={:.4f} acc={:.4f}'.format(
            epoch, n_loss/loss_ctr, n_acc/loss_ctr))
        val_loss, val_acc = n_loss/loss_ctr, n_acc/loss_ctr
        
        wandb.log({
            "Meta Train Error": train_loss, 
            "Meta Train Accuracy": train_acc,
            "Meta Valid Error": val_loss,
            "Meta Valid Accuracy": val_acc
            })
        
        if val_acc > best_valid_accuracy:
            best_valid_accuracy = val_acc
            torch.save(model.state_dict(), './checkpoint/protonet_og_custom.pt')
    ...
    model = Convnet()
    state_dict = torch.load('./checkpoint/protonet_og_custom.pt')
    ...
    test_acc = n_acc/loss_ctr
    print(f'\nMeta Test Accuracy: {test_acc}')
    wandb.log({
            "Meta Test Accuracy": test_acc, 
            })
